...
function getBeforeMonthTime(month: number) {
  const now = new Date();
  const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const beforeMonth = new Date(now.getFullYear(), now.getMonth() - month, now.getDate());

  return now.getTime() - (today.getTime() - beforeMonth.getTime());
}
